...
/// function ::= 'func' prototype expression
...
static std::unique_ptr<FunctionAST> ParseTopLevelExpr()
{
    ...
    if (auto E = ParseExpression())
    {
        ...
        args.push_back(std::make_pair("arg2", "float64"));
...
{
    // Specifying that it's a 32 bit signed int 
...
Value *IfExprAST::codegen()
{
    ...
    // Create blocks for the if and else cases.  Insert the 'then' block at the
    // end of the function.
...
Function *PrototypeAST::codegen()
{
    ...
    for (const auto &Arg : Args)
    {
        ...
        else if (Arg.second == "float64")
        {
            // If the argument type is "float64," use the double type.
...
static void HandleImport()
{
    ...
    {
        if (!ProtoAST->codegen())
            fprintf(stderr, "Error reading import");
        else
...
int main()
{
    ...
    KSDbgInfo.TheCU = DBuilder->createCompileUnit(
        dwarf::DW_LANG_C, DBuilder->createFile("test0.rs", "."),
        "Compiler", false, "", 0);
